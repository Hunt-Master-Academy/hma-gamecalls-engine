...
namespace huntmaster
{
    ...
    public:
        ...
        { // Add [[nodiscard]] to the Result struct itself
            T value;
            EngineStatus status;
        ...
        [[nodiscard]] EngineStatus processAudioChunk(int sessionId, const float *audioBuffer, int bufferSize);
...
extern "C"
{
    ...
    void destroyEngine(int engineId); // nodiscard is not useful on void
    [[nodiscard]] int loadMasterCall(int engineId, const char *masterCallId);
    [[nodiscard]] int startSession(int engineId);
    [[nodiscard]] int processAudioChunk(int engineId, int sessionId, const float *audioBuffer, int bufferSize, float sampleRate);
    [[nodiscard]] float getSimilarityScore(int engineId, int sessionId);
    void endSession(int engineId, int sessionId); // nodiscard is not useful on void
