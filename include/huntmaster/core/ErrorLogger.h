...
struct ErrorInfo {
    ...
    ErrorInfo(Component comp, ErrorSeverity sev, ErrorCategory cat, 
              const std::string& code, const std::string& msg)
        : component(comp), severity(sev), category(cat), errorCode(code), 
          message(msg), line(0), timestamp(std::chrono::system_clock::now()) {}
};
...
public:
     ...
     */
    void logError(Component component, ErrorSeverity severity, ErrorCategory category,
                  const std::string& errorCode, const std::string& message,
                  const std::string& details = "", const char* file = nullptr,
                  int line = 0, const char* function = nullptr);

    /**
     * @brief Log error with additional context data
     */
    void logErrorWithContext(Component component, ErrorSeverity severity, 
...
#define LOG_CRITICAL_ERROR(component, category, code, message) \
    huntmaster::ErrorLogger::getInstance().logError(component, \
        huntmaster::ErrorSeverity::CRITICAL, category, code, message, \
        "", __FILE__, __LINE__, __func__)

#define LOG_HIGH_ERROR(component, category, code, message) \
    huntmaster::ErrorLogger::getInstance().logError(component, \
        huntmaster::ErrorSeverity::HIGH, category, code, message, \
        "", __FILE__, __LINE__, __func__)

#define LOG_MEDIUM_ERROR(component, category, code, message) \
    huntmaster::ErrorLogger::getInstance().logError(component, \
        huntmaster::ErrorSeverity::MEDIUM, category, code, message, \
        "", __FILE__, __LINE__, __func__)

#define LOG_LOW_ERROR(component, category, code, message) \
    huntmaster::ErrorLogger::getInstance().logError(component, \
        huntmaster::ErrorSeverity::LOW, category, code, message, \
        "", __FILE__, __LINE__, __func__)

#define LOG_DETAILED_ERROR(component, severity, category, code, message, details) \
    huntmaster::ErrorLogger::getInstance().logError(component, severity, \
        category, code, message, details, __FILE__, __LINE__, __func__)

#define LOG_ERROR_WITH_CONTEXT(component, severity, category, code, message, context) \
    huntmaster::ErrorLogger::getInstance().logErrorWithContext(component, severity, \
        category, code, message, context, __FILE__, __LINE__, __func__)
