...
bool verifyProcessingConsistency(HuntmasterAudioEngine &engine)
{
    ...
    std::cout << "  Batch processing score: " << std::fixed << std::setprecision(8) << batchScore << std::endl;
    ...
    std::cout << "  Chunk processing score: " << std::fixed << std::setprecision(8) << chunkScore << std::endl;
...
int main()
{
    ...
    std::cout << "\nOverall: " << (allPass ? "ALL TESTS PASSED ✓" : "SOME TESTS FAILED ✗") << std::endl;
