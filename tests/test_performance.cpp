...
int main()
{
    ...
    for (int duration : testDurations)
    {
        ...
        std::cout << "  Chunks/second: " << std::fixed << std::setprecision(0)
                  << chunksPerSecond << std::endl;
    ...
    for (int i = 0; i < 100; ++i)
    {
        ...
        if (i % 10 == 0)
        {
            ...
            if (currentMemory > initialMemory + 50)
            { // Alert if > 50MB growth
    ...
    std::cout << "Memory stability: " << (memoryGrowth < 10 ? "Good" : "Potential leak detected") << std::endl;
