...
protected:
    ...
    void loadAvailableTestFiles() {
        ...
        ASSERT_GT(availableFiles_.size(), 0) 
            << "No test audio files found in " << masterCallsPath_;
    }
...
TEST_F(RealWildlifeCallAnalysisTest, MasterCallSimilarityScoring) {
    ...
    std::cout << "Master: " << masterFile.filename 
              << " vs Test: " << testFile.filename 
...
TEST_F(RealWildlifeCallAnalysisTest, CrossValidationBetweenCallTypes) {
    ...
    for (size_t i = 0; i < std::min(availableFiles_.size(), size_t(3)); ++i) {
        ...
        for (size_t j = 0; j < std::min(availableFiles_.size(), size_t(3)); ++j) {
            if (i == j) continue;  // Skip self-comparison for this test
    ...
    for (const auto& result : results) {
        std::cout << std::setw(15) << result.masterCall << " | "
                  << std::setw(13) << result.testCall << " | "
                  << std::setw(9) << std::fixed << std::setprecision(6) << result.similarity << " | "
                  << (result.expectedMatch ? "Yes" : "No") << "\n";
...
TEST_F(RealWildlifeCallAnalysisTest, PerformanceBenchmarkWithRealAudio) {
    ...
    float audioLengthMs = (audioData.samples.size() / static_cast<float>(audioData.sample_rate)) * 1000.0f;
    double realTimeRatio = averageTime / audioLengthMs;
    
    std::cout << "\n=== Performance Benchmark Results ===\n";
    std::cout << "Audio File: " << testFile.filename << "\n";
    std::cout << "Audio Length: " << audioLengthMs << " ms (" 
              << (audioLengthMs / 1000.0f) << " seconds)\n";
    ...
    std::cout << "Throughput: " << (audioData.samples.size() / (averageTime / 1000.0)) << " samples/second\n";
    
    // Performance assertions
    EXPECT_LT(realTimeRatio, 1.0) << "Processing should be faster than real-time for efficient operation";
...
TEST_F(RealWildlifeCallAnalysisTest, ErrorHandlingWithRealAudio) {
    ...
    if (availableFiles_.size() > 0) {
        std::string masterCallId = availableFiles_[0].filename.substr(0, availableFiles_[0].filename.find_last_of('.'));
        ...
        if (goodLoadResult == UnifiedAudioEngine::Status::OK) {
            ...
            EXPECT_TRUE(shortScoreResult.isOk() || 
                       shortScoreResult.error() == UnifiedAudioEngine::Status::INSUFFICIENT_DATA);
        }
    }
