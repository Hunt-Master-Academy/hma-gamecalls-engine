...
// Test harness converted to Google Test format
TEST(TestHarnessTest, BasicEngineOperations) {
    ...
    if (engine.loadMasterCall("buck_grunt") != HuntmasterAudioEngine::EngineStatus::OK) {
        ...
        engine.shutdown();
        FAIL() << "Failed to load master call";
    }
    ...
    if (userAttemptAudio.empty()) {
        engine.shutdown();
        FAIL() << "Failed to load user attempt audio file";
    }
    ...
    if (!sessionResult.isOk()) {
        ...
        engine.shutdown();
        FAIL() << "Failed to start a realtime session";
    }
    ...
    if (!scoreResult.isOk()) {
        ...
        engine.endRealtimeSession(sessionId);
        engine.shutdown();
        FAIL() << "Failed to get similarity score";
    }
