...
std::span<const T> typedArrayToSpan(emscripten::val array) {
    ...
    return std::span<const T>(reinterpret_cast<const T*>(array["byteOffset"].as<uintptr_t>()),
                              length);
}
}  // namespace
...
//     EM_ASM_({ console.log('HuntmasterEngine Initializing with SR:', $0, 'Frame:', $1, 'MFCCs:',
//     $2); }, sampleRate, frameSize, mfccCoeffs);
//     // pimpl_->engine_ = std::make_unique<huntmaster_engine::HuntmasterEngine>(config); // Actual
//     initialization pimpl_->engine_ = std::make_unique<huntmaster_engine::HuntmasterEngine>(); //
//     Placeholder pimpl_->processor_ =
//     std::make_unique<huntmaster_engine::RealtimeAudioProcessor>(); // Placeholder return true;
// }
bool WASMInterface::initialize(int sampleRate, int frameSize, int mfccCoeffs) {
    EM_ASM_(
        { console.log('HuntmasterEngine Initializing with SR:', $0, 'Frame:', $1, 'MFCCs:', $2); },
        sampleRate,
        frameSize,
        mfccCoeffs);

    // Create proper configuration
    huntmaster::PlatformEngineConfig config{.sample_rate = static_cast<size_t>(sampleRate),
    ...
    huntmaster::RealtimeAudioProcessor::Config proc_config{.ring_buffer_size = 256,
                                                           .chunk_size =
...
bool WASMInterface::loadMasterCall(const std::string& callName, emscripten::val audioData) {
    ...
    EM_ASM_(
        { console.log('Loading master call:', UTF8ToString($0), 'with', $1, 'samples.'); },
        callName.c_str(),
        span.size());
...
bool WASMInterface::enqueueAudioBuffer(emscripten::val buffer) {
    ...
    try
    {
        ...
        if (!success) EM_ASM_({ console.warn('Enqueue audio buffer failed: Buffer full or other
    issue.'); }); return success;
    }
    catch (...)
    {
        return false;
    ...
    */
    ...
    auto audioSpan = typedArrayToSpan<float>(buffer);
    // bool success = pimpl_->processor_->tryEnqueueAudio(audioSpan); // This would be the actual
    // call if (!success) EM_ASM_({ console.warn('Enqueue audio buffer failed: Buffer full or other
    // issue.'); }); return success;
...
bool WASMAudioWorker::initialize(uintptr_t sharedBufferPtr, size_t bufferSize) {
    if (sharedBufferPtr == 0 || bufferSize == 0 || bufferSize % sizeof(float) != 0) {
        EM_ASM_(
            { console.error('WASM Audio Worker initialize failed: Invalid pointer or size.'); });
        return false;
    }
