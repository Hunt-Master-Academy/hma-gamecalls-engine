...
namespace huntmaster
{
    ...
    public:
            ...
            : config_(config), adaptive_threshold_(config.energy_threshold)
        {
            ...
            samples_per_window_ = (config.sample_rate * config.window_duration.count()) / 1000;
            energy_window_size_ = std::max(size_t(10),
                                           size_t(1000 / config.window_duration.count()));

            size_t pre_buffer_frames = config.pre_buffer.count() / config.window_duration.count();
            size_t post_buffer_frames = config.post_buffer.count() / config.window_duration.count();

            pre_buffer_.resize(pre_buffer_frames);
            post_buffer_.resize(post_buffer_frames);
        }

        [[nodiscard]] float computeEnergy(std::span<const float> audio)
        {
        ...
        void updateAdaptiveThreshold(float energy)
        {
            ...
            if (energy_history_.size() >= 5)
            {
                ...
                adaptive_threshold_ = std::max(config_.energy_threshold,
                                               noise_floor * 3.0f);
            }
        }

        [[nodiscard]] VADResult processStateMachine(float energy,
                                                    std::span<const float> audio)
        {
            ...
            std::visit([&](auto &&state_handler)
                       {
            using T = std::decay_t<decltype(state_handler)>;
            ...
            else if (state_ == VADState::VOICE_ONSET) {
                if (is_voice) {
            ...
            else if (state_ == VADState::VOICE_ACTIVE) {
                if (is_voice) {
                    last_active_time_ = now;
                    result.is_active = true;
                } else {
            ...
            else if (state_ == VADState::VOICE_OFFSET) {
            ...
            if (result.is_active && state_ == VADState::VOICE_ACTIVE)
            {
        ...
        void updateBuffers(std::span<const float> audio)
        {
            std::vector<float> frame(audio.begin(), audio.end());

            if (state_ == VADState::SILENCE || state_ == VADState::VOICE_ONSET)
            {
                pre_buffer_.push_back(std::move(frame));
                if (pre_buffer_.size() > config_.pre_buffer.count() /
            ...
            if (state_ == VADState::VOICE_OFFSET)
            {
                ...
                if (post_buffer_.size() > config_.post_buffer.count() /
                                              config_.window_duration.count())
                {
    ...
    std::expected<VADResult, VADError>
    ...
    std::chrono::milliseconds VoiceActivityDetector::getActiveDuration() const noexcept
    {
        ...
        if (pimpl_->state_ == VADState::VOICE_ACTIVE)
        {
            return duration_cast<milliseconds>(
                steady_clock::now() - pimpl_->voice_start_time_);
        }
