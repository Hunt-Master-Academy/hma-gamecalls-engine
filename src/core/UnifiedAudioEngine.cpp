...
UnifiedAudioEngine::Result<std::unique_ptr<UnifiedAudioEngine>> UnifiedAudioEngine::create() {
    ...
    try {
        ...
        ComponentErrorHandler::MemoryErrors::logMemoryAllocationError(
            "ENGINE_MEMORY_ALLOC_FAILED", "Failed to allocate memory for UnifiedAudioEngine");
        return {nullptr, Status::INIT_FAILED};

    } catch (const std::exception& e) {
        ComponentErrorHandler::UnifiedEngineErrors::logInitializationError(
            "ENGINE_INIT_EXCEPTION",
            "Exception during UnifiedAudioEngine creation: " + std::string(e.what()));
        return {nullptr, Status::INIT_FAILED};

    } catch (...) {
        ComponentErrorHandler::UnifiedEngineErrors::logInitializationError(
            "ENGINE_INIT_UNKNOWN_EXCEPTION",
            "Unknown exception during UnifiedAudioEngine creation");
        return {nullptr, Status::INIT_FAILED};
    }
}
...
UnifiedAudioEngine::Status
UnifiedAudioEngine::Impl::destroySession(SessionId sessionId) {
    LOG_DEBUG(Component::UNIFIED_ENGINE,
                ...
                try {
                    ...
                    if (session) {
                        LOG_DEBUG(Component::UNIFIED_ENGINE,
                                  "Destroying session " + std::to_string(sessionId)
                                      + " with sample rate: "
                                      + std::to_string(session->sampleRate));
                    }
...
UnifiedAudioEngine::Status
UnifiedAudioEngine::Impl::loadMasterCall(SessionId sessionId,
                                         std::string_view masterCallId) {
    ...
    DrWavRAII audioData(rawData);
                ...
                auto featuresResult =
                    session->mfccProcessor->extractFeaturesFromBuffer(monoSamples, 256);
                if (!featuresResult) {
                ...
                if (session->realtimeScorer) {
                    if (!session->realtimeScorer->setMasterCall(audioFilePath)) {
                        ...
                        std::cerr
                            << "[UnifiedAudioEngine] Failed to set master call in RealtimeScorer"
                            << std::endl;
#endif
            ...
            UnifiedAudioEngine::Impl::processAudioChunk(SessionId sessionId,
                ...
                if (audioBuffer.size() > 1000000) {  // Reasonable upper limit
                    ComponentErrorHandler::UnifiedEngineErrors::logParameterValidationError(
                        ...
                        "Excessively large audio buffer: " + std::to_string(audioBuffer.size())
                            + " samples");
                    LOG_WARN(Component::UNIFIED_ENGINE,
                             "Processing very large audio buffer: "
                                 + std::to_string(audioBuffer.size()) + " samples");
                }
                ...
                if (!session) {
                    ComponentErrorHandler::UnifiedEngineErrors::logSessionError(
                        "SESSION_NOT_FOUND_PROCESSING",
                        "Session not found during audio processing");
                    return Status::SESSION_NOT_FOUND;
                }
                ...
                try {
                    ...
                    if (session->realtimeScorer) {
                        auto result = session->realtimeScorer->processAudio(
                            audioBuffer, 1);  // Assume mono for now
                        if (!result) {
                            ComponentErrorHandler::UnifiedEngineErrors::logProcessingError(
                        ...
                        for (size_t i = 0; i + frameSize <= audioBuffer.size(); i += frameSize) {
                            ...
                            try {
                                ...
                                ComponentErrorHandler::UnifiedEngineErrors::logProcessingError(
                                    ...
                                    "VAD processing failed: " + std::string(e.what()));
                                // Continue processing remaining frames
                            }
                        }

                        LOG_TRACE(Component::UNIFIED_ENGINE,
                                  "VAD processed " + std::to_string(processedSamples)
                                      + " samples for session " + std::to_string(sessionId));
                    } else {
            ...
            if (!session->currentSegmentBuffer.empty()) {
                try {
                    ...
                    ComponentErrorHandler::MFCCProcessorErrors::logFeatureExtractionError(
                        ...
                        "MFCC feature extraction failed: " + std::string(e.what()));
                    return Status::PROCESSING_ERROR;
                }
            }

            LOG_TRACE(Component::UNIFIED_ENGINE,
                      "Audio chunk processed successfully for session "
                          + std::to_string(sessionId));
            return Status::OK;
            }
            catch (const std::exception& e) {
                ...
                ComponentErrorHandler::UnifiedEngineErrors::logProcessingError(
                    "AUDIO_PROCESSING_UNKNOWN_EXCEPTION",
                    "Unknown exception during audio chunk processing");
                return Status::PROCESSING_ERROR;
            }
            }
            ComponentErrorHandler::UnifiedEngineErrors::logProcessingError(
                ...
                "Unknown exception during audio chunk processing",
                sessionId,
                {{"buffer_size", std::to_string(audioBuffer.size())}});
            return Status::PROCESSING_ERROR;
            }
            }
            }
            ...
            UnifiedAudioEngine::Impl::getSimilarityScore(SessionId sessionId) {
                ...
                if (session->masterCallFeatures.empty() || session->sessionFeatures.empty()) {
                    std::cerr << "DEBUG: masterCallFeatures.size() = "
                              << session->masterCallFeatures.size()
                              << ", sessionFeatures.size() = " << session->sessionFeatures.size()
                              << std::endl;
                    return {0.0f, Status::INSUFFICIENT_DATA};
                }
                ...
                const float distance = session->dtwComparator->compare(session->masterCallFeatures,
            ...
            void UnifiedAudioEngine::Impl::extractMFCCFeatures(SessionState& session) {
                ...
                auto featuresResult = session.mfccProcessor->extractFeaturesFromBuffer(
                    session.currentSegmentBuffer, hopSize);
                if (featuresResult) {
                    session.sessionFeatures.insert(session.sessionFeatures.end(),
                                                   featuresResult->begin(),
                                                   featuresResult->end());
                }
            ...
            UnifiedAudioEngine::Impl::SessionState*
            UnifiedAudioEngine::Impl::getSession(SessionId sessionId) {
                std::shared_lock lock(sessionsMutex_);
                auto it = sessions_.find(sessionId);
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::configureVAD(SessionId sessionId, const VADConfig& config) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            bool UnifiedAudioEngine::Impl::isVADActive(SessionId sessionId) const {
                ...
                return session->vadEnabled && session->vadConfig.enabled
                       && session->vad->isVoiceActive();
            }

            UnifiedAudioEngine::Status UnifiedAudioEngine::Impl::enableVAD(SessionId sessionId,
                                                                           bool enable) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::unloadMasterCall(SessionId sessionId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Impl::getSessionDuration(SessionId sessionId) const {
                ...
                auto duration =
                    std::chrono::duration_cast<std::chrono::milliseconds>(now - session->startTime);
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::startRecording(SessionId sessionId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::stopRecording(SessionId sessionId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Result<std::string>
            UnifiedAudioEngine::Impl::saveRecording(SessionId sessionId,
                                                    std::string_view filename) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return {"", Status::SESSION_NOT_FOUND};
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::startMemoryRecording(SessionId sessionId,
                                                           double maxDurationSeconds) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
                ...
                if (maxDurationSeconds > 0.0) {
                    config.maxMemoryBufferSize = static_cast<size_t>(
                        maxDurationSeconds * session->sampleRate * config.channels);
            ...
            UnifiedAudioEngine::Impl::getRecordedAudioData(SessionId sessionId) const {
                ...
                if (session->audioRecorder->getRecordingMode()
                        != AudioRecorder::RecordingMode::MEMORY_BASED
                    && session->audioRecorder->getRecordingMode()
                           != AudioRecorder::RecordingMode::HYBRID) {
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::clearRecordingBuffer(SessionId sessionId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Impl::getRecordingMode(SessionId sessionId) const {
                ...
                AudioRecorder::RecordingMode recorderMode =
                    session->audioRecorder->getRecordingMode();
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::setRecordingMode(SessionId sessionId, RecordingMode mode) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Impl::getMemoryBufferInfo(SessionId sessionId) const {
                ...
                AudioRecorder::MemoryBufferStats stats =
                    session->audioRecorder->getMemoryBufferStats();
                ...
                info.freeFrames = stats.maxSamples > stats.currentSamples
                                      ? stats.maxSamples - stats.currentSamples
                                      : 0;
            ...
            bool UnifiedAudioEngine::Impl::isRecording(SessionId sessionId) const {
                ...
                if (!session)
                    return false;
                return session->isRecording && session->audioRecorder
                       && session->audioRecorder->isRecording();
            }
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::playMasterCall(SessionId sessionId,
                                                     std::string_view masterCallId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
                ...
                const std::string audioFilePath =
                    masterCallsPath_ + std::string(masterCallId) + ".wav";
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::playRecording(SessionId sessionId,
                                                    std::string_view filename) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            bool UnifiedAudioEngine::Impl::isPlaying(SessionId sessionId) const {
                ...
                if (!session)
                    return false;
                return session->isPlaying && session->audioPlayer
                       && session->audioPlayer->isPlaying();
            }
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::setPlaybackVolume(SessionId sessionId, float volume) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
            ...
            UnifiedAudioEngine::Status
            UnifiedAudioEngine::Impl::endRealtimeSession(SessionId sessionId) {
                SessionState* session = getSession(sessionId);
                if (!session)
                    return Status::SESSION_NOT_FOUND;
    ...
    UnifiedAudioEngine::Result<std::string>
    UnifiedAudioEngine::Impl::exportScoringHistoryToJson(SessionId sessionId,
                                                         size_t maxCount) {
        const SessionState* session = getSession(sessionId);
        if (!session)
            return {std::string{}, Status::SESSION_NOT_FOUND};
...
UnifiedAudioEngine::Status UnifiedAudioEngine::loadMasterCall(SessionId sessionId, std::string_view masterCallId) {
    return pImpl->loadMasterCall(sessionId, masterCallId);
}
...
UnifiedAudioEngine::Result<std::string> UnifiedAudioEngine::getCurrentMasterCall(SessionId sessionId) const {
    return pImpl->getCurrentMasterCall(sessionId);
}

UnifiedAudioEngine::Status UnifiedAudioEngine::processAudioChunk(SessionId sessionId, std::span<const float> audioBuffer) {
    return pImpl->processAudioChunk(sessionId, audioBuffer);
}
...
UnifiedAudioEngine::Status UnifiedAudioEngine::setRealtimeScorerConfig(SessionId sessionId, const RealtimeScorerConfig& config) {
    return pImpl->setRealtimeScorerConfig(sessionId, config);
}

UnifiedAudioEngine::Result<RealtimeScoringResult> UnifiedAudioEngine::getRealtimeScoringResult(SessionId sessionId) {
...
UnifiedAudioEngine::Result<std::string> UnifiedAudioEngine::exportFeedbackToJson(SessionId sessionId) {
    return pImpl->exportFeedbackToJson(sessionId);
}

UnifiedAudioEngine::Result<std::string> UnifiedAudioEngine::exportScoringHistoryToJson(SessionId sessionId, size_t maxCount) {
    return pImpl->exportScoringHistoryToJson(sessionId, maxCount);
}
