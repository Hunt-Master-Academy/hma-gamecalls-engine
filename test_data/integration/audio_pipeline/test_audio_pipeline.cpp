...
TEST_F(AudioPipelineTest, FullPipelineStreamProcessing) {
    ...
    [[maybe_unused]] auto resetStatus = engine->resetSession(sessionId);
    ...
    [[maybe_unused]] auto resetStatus2 = engine->resetSession(sessionId);
    [[maybe_unused]] auto vadStatus = engine->disableVAD(sessionId);

    pos = 0;
    while (pos < audio_stream.size()) {
        size_t current_chunk_size = std::min(chunk_size, audio_stream.size() - pos);
        std::span<const float> audio_chunk(audio_stream.data() + pos, current_chunk_size);
        [[maybe_unused]] auto processStatus = engine->processAudioChunk(sessionId, audio_chunk);
        pos += current_chunk_size;
    }
